...
You classified the issue as **`spec_amendment`**: a requirement, interface contract, or ADR is
wrong — unbuildable, self-contradictory, or contradicting the capability that drives it, while
that capability is sound. Amend the spec, then return to SKILL.md Step 4.
...
- where the amended statement appears verbatim in the `task_id` contract in `$sprint_artifact`
  ...
  Change nothing else in the contract: its acceptance criteria and task shape are not yours on
  this path.
