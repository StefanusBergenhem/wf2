...
the durable ADR dir, and the capabilities, charter, plan, and learnings homes. It also adds
...
3. **Confirm with the human before writing.** Present each proposal with its evidence
   ...
   every task it gates, and a `paths.tests` that misses a root under-reports the shipped
