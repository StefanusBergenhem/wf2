...
class GitopsTest(support.TempProject):
    ...
    def test_conflicting_merge_is_detected_named_and_left_clean(self):
        self.git.start_branch("sprint/s1", "main")
        ...
        result = self.git.merge("task/T2", "T2: merge")
        self.assertFalse(result.ok)
        self.assertTrue(result.conflict)
        self.assertIn("shared.txt", result.files)
        # the branch is left usable so the rest of the batch can still merge
        self.assertTrue(self.git.is_clean())
